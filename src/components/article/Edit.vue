  ...
  <div>
    <section class="site-section">
      <div class="container">
        <div class="row mb-4">
            <div class="col-md-6">
                <h1 class="mb-4">Add Articles
                  <small>
                      <router-link :to="{name:'view-articles'}" 
                        :class="'btn btn-primary btn-xs rounded'">View</router-link>
                  </small>
                </h1>
            </div>
        </div>
    ...
    async submit(){
      ...
      .then(data => {
          ...
          if(result.errFlag == 0)
          {
            ...
            {
              this.$router.replace({ name:'make-payment', params:{ article_id : result.article_id  } });
              location.reload();
            }
            else{
