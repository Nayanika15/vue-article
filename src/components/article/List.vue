          ...
          <div class="col-md-12 col-lg-12 main-content">
            <v-data-table
              ...
              <template v-slot:item.paid_status="{ item }">
                <span class="text-success" v-if="item.paid_status == 1"> Paid </span>
              ...
              <template v-slot:item.action="{ item }">
                <router-link class="btn btn-primary" :to="{ name:'edit-article' , params:{ id: item.id }}"> <i class='fa fa-edit'></i> </router-link>
...
export default {
  ...
  methods:{
    makeFeatured(item){
      this.$http.get('article/feature/'+ item.id)
        .then( response => {
            return response.json();
        })
        .then( data => {
    ...
    deleteArticle(item){
      ...
      {
        this.$http.get('article/delete/'+ item.id)
        .then(response => {
            return response.json();
        })
        .then(data => {
