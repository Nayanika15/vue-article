<template>
    <div class="col-md-12 col-lg-4 sidebar">
        <div class="sidebar-box" v-if="articles_count">
            ...
            <ul>
                <li 
                    v-for="(article, id) in popular_articles"
                    :key="id">
                <router-link :to="{ name: 'articleDetail', params:{ slug:article.slug}}">
                    <img :src="article.category_image" 
        ...
        <div class="sidebar-box" v-if="categories_count">
            ...
            <li v-for="(category, id) in active_categories" :key="id">
                <router-link :to="{name:'categoryDetail', params:{ slug : category.slug  } }"> {{ category.name}} <span>( {{ category.articles_count }})</span></router-link></li>
            </ul>
        </div>
        <!-- END sidebar-box -->
    </div>
